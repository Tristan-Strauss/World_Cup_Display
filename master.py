...
def send_command(command):
    ser.write(f"{command}\n".encode())
    print(f"[DEBUG] Sent command to slave: {command}")
...
# Get input
# Check if input valid year
# Check if year local
# Set pin high or low if local
# Send command if not local (eg command= 2034_ON or 2034_OFF)